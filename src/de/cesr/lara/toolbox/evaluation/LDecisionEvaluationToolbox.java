...
public class LDecisionEvaluationToolbox {
	...
	public static <A extends LaraAgent<A, BO>, BO extends LaraBehaviouralOption<A, BO>> LaraPreprocessorFactory<A, BO> getPostProcessPreprocessBuilder(
			LaraPreprocessorConfiguration<A, BO> configurator,
			LaraDecisionConfiguration dBuilder) {

		LaraPreprocessorConfiguration<A, BO> newConfigurator = configurator
				.clone();
		// / force preprocessor to select deliberative decision making mode
		// (assign LDeliberativeDeciderFactory):
		configurator.setDecisionModeSelector(new LaraDecisionModeSelector<A>() {
			@Override
			public void selectDecisionMode(A agent,
					LaraDecisionConfiguration dBuilder) {
				agent.getLaraComp()
						.getDecisionData(dBuilder)
						.setDeciderFactory(
								LDeliberativeDeciderFactory
										.<A, BO> getFactory((Class<A>) agent
												.getClass()));
			}
		}, dBuilder);
		configurator.setBOCollector(new LContributingBOPreselector<A, BO>(),
				dBuilder);
		configurator.setBOAdapter(new LDefaultLBOUpdaterBuilder<A, BO>(),
				dBuilder);
		configurator.setPreferenceUpdater(new LPseudoPrefereceUpdater<A>(),
				dBuilder);
	...
	public <A extends LaraAgent<A, BO>, BO extends LaraBehaviouralOption<A, BO>> void evaluateSelectedBoAgainstOpimalBo(
		...
		// exchange dBuilder because of autonomous random stream for
		...
		getPostProcessPreprocessBuilder(configurator, postProcessDBuilder)
				.getPreprocessor(postProcessDBuilder).preprocess(
						LaraBOPreselector.LAccuracy.ACCURATE, agent);
